...
model = YOLO('machineLearning/Lithology/best.pt','v8')

# Display model information (optional)
# model.info()

# Run inference with the YOLOv8n model on the 'bus.jpg' image
# results = model.predict(source='coresamples.jpg',conf=0.60,save=True)
# results = model.predict(source='backend/public/Images/1704349535118_WhatsApp Image 2023-12-19 at 7.30.11 PM.jpeg',conf=0.60,save=True)

inputPath  =  sys.argv[1]
outputPath = sys.argv[2] + sys.argv[3] + "_predication.png"


results = model.predict(source=inputPath, conf=0.60, save=True)


print(results)
# results = model.predict(source=sys.argv[1],conf=0.60,save=True)
...
# # cv2.imwrite('runs/detection_image.jpg', orig_img_bgr)
